...
///
library windows_pos_print.generator;
...
class Generator {
  ...
  double _getCharWidth(PosStyles styles, {int? maxCharsPerLine}) {
    final charsPerLine = _getCharsPerLine(styles, maxCharsPerLine);
    final charWidth = (paperWidthMM / charsPerLine) * styles.width;
  ...
  List _getLexemes(String text) {
    ...
    final isLexemeChinese = <bool>[];
    var start = 0;
    var end = 0;
    var curLexemeChinese = _isChinese(text[0]);
  ...
  bool _isChinese(String ch) => ch.codeUnitAt(0) > 255;
  ...
  List<int> row(List<PosColumn> cols) {
    ...
    for (int i = 0; i < cols.length; ++i) {
      ...
      final charWidth = _getCharWidth(cols[i].styles);
      final fromPos = _colIndToPosition(colInd);

      final toPos =
          _colIndToPosition(colInd + cols[i].width) - spaceBetweenRows;

      final maxCharactersNb = ((toPos - fromPos) / charWidth).floor();
      ...
      if (!cols[i].containsChinese) {
        ...
        if (realCharactersNb > maxCharactersNb) {
          ...
          final encodedToPrintNextRow = encodedToPrint.sublist(maxCharactersNb);
        ...
        for (int p = 0; p < cols[i].text.length; ++p) {
          final w = _isChinese(cols[i].text[p]) ? 2 : 1;
        ...
        final toPrintNextRow = cols[i].text.substring(splitPos);
        final toPrint = cols[i].text.substring(0, splitPos);
        ...
        final lexemes = list[0];
        final isLexemeChinese = list[1];
  ...
  List<int> image(Image imgSrc, {PosAlign align = PosAlign.center}) {
    ...
    final heightPx = imageRotated.height;
    const densityByte = 1 + 32;

    final header = List.from(cBitImg.codeUnits);
  ...
  List<int> imageRaster(
    ...
    final resterizedData = _toRasterFormat(image);
    ...
    if (imageFn == PosImageFn.bitImageRaster) {
      ...
      final header = List.from(cRasterImg2.codeUnits);
    ...
    //
      ...
      final header1 = List.from(cRasterImg.codeUnits);
      ...
      final header2 = List.from(cRasterImg.codeUnits);
  ...
  /// Open cash drawer
  ///
  List<int> drawer({PosDrawer pin = PosDrawer.pin2}) => pin == PosDrawer.pin2
  ...
  List<int> hr({String ch = '-', int? len, int linesAfter = 0}) {
    var bytes = <int>[];
    var n = len ?? _maxCharsPerLine ?? _getMaxCharsPerLine(_styles.fontType);
    var ch1 = ch.length == 1 ? ch : ch[0];
